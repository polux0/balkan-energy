...
const filename = "/src/utils/scripts/data/production/auctions-monthly/auctions-monthly.xls";
const XLXS = require('xlsx');
const workbook = XLXS.readFile(filename);
const sheetNameList = workbook.SheetNames;
const {auctionMonthly} = require('../../../../models')
...
async function compare(object1){
  ...
  let objectComparedTo = null;
       ...
       objectComparedTo = await db.sequelize.query('SELECT * FROM auctionMonthly WHERE timestamp = :timestamp AND code = :code', {
        replacements: {timestamp: object1.timestamp, code: object1.code},
        model: db.auctionMonthly,
        mapToModel:true
      });
      //change in other scripts as well;
      if(typeof objectComparedTo[0] === 'undefined'){
        return auctionMonthly.create(object1)
      }      
      else if(object1.capacity !== objectComparedTo[0].dataValues.capacity || object1.value !== objectComparedTo[0].dataValues.value){
        return auctionMonthly.update({capacity: object1.capacity, value: object1.value}, {where:{id:objectComparedTo[0].dataValues.id}})
      }
    ...
    if(header.startsWith('capacity')){
      ...
      result.map(async value => {
        ...
        let object = {
             ...
             code: `${countries}`,
             displayCode: `auction monthly ${countries}`,
...
try {
  await auctionMonthly.bulkCreate(finalArray)
